...
public class FitnessFunctionPSO3 extends FitnessFunction {
     ...
     * tasklength：这表示要执行的每个cloudlet的任务长度
     * commcost：这是单位时间内从一个任务到另一个任务的结果的通信成本
	 */

    /** 该矩阵表示在vm上运行cloudlet所需的执行时间. */
    private static double[][] executiontimematrix;
    /** 该矩阵表示输出文件从一个任务到另一个任务的通信时间. */
    private static double[][] communicationtimematrix;
    /** 该矩阵表示从一个cloudlet传输到另一个cloudlet的数据量. */
    private static double[][] taskoutputfilematrix;
    /** 这个矩阵表示从一个cloudlet到另一个cloudlet的数据传输速度. */
    private static double[][] datatransfermatrix;
    /** 这个数组表示每个cloudlet的等待时间. */
    private static double[] waittime;
    /** 这表示任务之间的依赖关系. */
    public int graph[][];
    /** 这个矩阵表示每个任务生成的输出文件的大小. */
    int outputfilesize[];
    /** 这是被调用的每个VM的mips等级（每秒处理的指令数为百万）. */
    int mips[];
    /** 这是每单位时间在不同VM上执行的成本. */
    double execcost[];
    /** 这是单位时间内不同VM上的waitcost成本. */
    double waitcost[];
    /** 这表示要执行的每个cloudlet的任务长度. */
    int tasklength[];
    /** 这是单位时间内从一个任务到另一个任务的结果的通信成本. */
    double commcost[][] = new double[Constants3.NoOfTasks][Constants3.NoOfTasks];
    //适应度函数
    FitnessFunctionPSO3(double execcost[], double waitcost[], int mips[], int outputfilesize[],
    ...
    private void initializeMatrices() {
        ...
        executiontimematrix = new double[Constants3.NoOfTasks][Constants3.NoOfVMs];
        communicationtimematrix = new double[Constants3.NoOfTasks][Constants3.NoOfTasks];
        taskoutputfilematrix = new double[Constants3.NoOfTasks][Constants3.NoOfTasks];
        datatransfermatrix = new double[Constants3.NoOfTasks][Constants3.NoOfTasks];
        waittime = new double[Constants3.NoOfTasks];
        /*
         * Calculation of Execution Time of each task i on vm j
         * 计算vm j上每个任务i的执行时间
         */
        for (int i = 0; i < Constants3.NoOfTasks; i++) {
            for (int j = 0; j < Constants3.NoOfVMs; j++) {
        ...
        //任务传输的数据量
        for (int i = 0; i < Constants3.NoOfTasks; i++) {
            for (int j = i; j < Constants3.NoOfTasks; j++) {
                if (i == j)
...
//		printmatrices();
    ...
    public double calcMakespan(double[] position){
        double makespan = 0.0;
        double[] vmworkingtime = new double[Constants3.NoOfVMs];
        for (int i = 0; i< Constants3.NoOfTasks; i++){
    ...
    public double calculatecost(double[] position) {
        double cost = 0.0;
        double[] vmworkingcost = new double[Constants3.NoOfVMs];
        /*
